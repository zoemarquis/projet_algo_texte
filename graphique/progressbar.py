...
class ProgressBar:
    def __init__(self, frame_parent, fenetre, grid_row, grid_column):
        ...
        self.style.configure("Custom.Horizontal.TProgressbar", troughcolor=theme.couleur_frame, background=theme.couleur_selection)
        self.loadbar = ttk.Progressbar(frame_parent, orient='horizontal', mode='determinate', style="Custom.Horizontal.TProgressbar")
        self.loadbar.grid(row=grid_row, column=grid_column, sticky="ewns", pady=(0, 30))
        self.fenetre = fenetre

        # bouton start / stop
        self.bouton = ttk.Button(frame_parent, text="Start", command=self.toggle_progress, style="Custom.TButton")
    ...
    def update_progress(self):
        ...
        if self.progress_running and current_value < 100:
            self.loadbar['value'] += 1  # Incrémente la valeur de la barre de progression
            # Planifie la mise à jour de la progression après un délai
            self.fenetre.after(100, self.update_progress)  # Continue à appeler update_progress toutes les 100 ms
    ...
    def toggle_progress(self):
        self.progress_running = not self.progress_running  # Bascule l'état de progression
